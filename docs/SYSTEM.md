...
|---|---|---|
| **Cloud** | Email · Calendar · Files · Meetings | **Split in two — processing shipped, auth/pull a recipe.** The **processing** half is built + tested code ([`ingest/cloud/`](../ingest/cloud/), `python -m ingest.cloud.refresh`): a connector dump → the Gmail sent-correspondent filter → the shared spine → genesis → proposals-only, gated by the ONE allowlist. The **auth + pull** half stays a scheduled read-only routine you stand up from [CLOUD-ROUTINE.md](CLOUD-ROUTINE.md) (paste-in prompt + rails), because that part is welded to your assistant platform. The routine also writes the correspondents file the local lanes consume; the email adapter both halves reuse (sent-correspondent harvest + spam-drop) is built + tested in [`ingest/adapters/email_source.py`](../ingest/adapters/email_source.py). |
...
| **Connectors — cloud lane, PROCESSING** (Gmail sent-filter → spine → genesis → proposals) | ✅ Yes | **Built + tested in [`ingest/cloud/`](../ingest/cloud/)** (`python -m ingest.cloud.refresh` on a connector dump; 28 tests). Reuses the canonical correspondent harvest + the ONE allowlist; proposals-only, no OAuth/network/git. |
| **Connectors — cloud lane, AUTH + PULL** (the scheduled read-only routine) | ⚠️ Recipe | **Specified, not shipped code**: the connector auth/pull stays a scheduled routine you stand up from [CLOUD-ROUTINE.md](CLOUD-ROUTINE.md) (welded to your assistant platform). It feeds the processing code above. |
